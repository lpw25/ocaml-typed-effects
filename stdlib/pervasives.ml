...
let abs x = Int_compare.(if x >= 0 then x else -x)
...
type ('a, 'b, 'c, 'd, 'e, 'f) format6 =
  ('a, 'b, 'c, 'd, 'e, 'f, ![]) format6e

type ('a, 'b, 'c, 'd, !p) format4e = ('a, 'b, 'c, 'c, 'c, 'd, !p) format6e

type ('a, 'b, 'c, 'd) format4 = ('a, 'b, 'c, 'd, ![]) format4e

type ('a, 'b, 'c, !p) formate = ('a, 'b, 'c, 'c, !p) format4e

type ('a, 'b, 'c) format = ('a, 'b, 'c, ![]) formate
...
let exit_function = ref (flush_all : unit -> unit)

let at_exit f =
  let g = !exit_function in
  exit_function := (fun () -> f(); g())

let do_at_exit : unit -> unit = fun () -> (!exit_function) ()
