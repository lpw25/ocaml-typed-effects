...
let type_padding :
  type a b c d e f x y effect p q.
...
(* Raise Type_mismatch in case of type mismatch. *)
let type_padprec :
  type a b c d e f x y z effect p q.
...
let fmt_ebb_of_string ?legacy_behavior str =
  ...
  *)
  ...
  fun pct_ind str_ind end_ind plus sharp space ign pad prec padprec symb ->
    ...
    let check_no_0 symb (type a) (type b) (effect s)
                   (pad : (a, b, s) padding) =
      match pad with
      | No_padding -> pad
    ...
    let opt_of_pad c (type a) (type b) (effect s)
  ...
  and check_open_box :
    type a b c d e f effect p. (a, b, c, d, e, f, p) fmt -> unit =
