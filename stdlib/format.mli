...
val fprintf :
  formatter -> ('a, formatter, unit, !~) formate ~> 'a
...
val printf : ('a, formatter, unit, !~) formate ~> 'a
(** Same as [fprintf] above, but output on [std_formatter]. *)

val eprintf : ('a, formatter, unit, !~) formate ~> 'a
...
val asprintf :
  ('a, formatter, unit, string, !~) format4e ~> 'a
(** Same as [printf] above, but instead of printing on a formatter,
   returns a string containing the result of formatting the arguments.
...
val ifprintf : formatter -> ('a, formatter, unit, !~) formate ~> 'a
...
val bprintf :
  Buffer.t -> ('a, formatter, unit, !~) formate ~> 'a
  [@@ocaml.deprecated]
