...
let check_partial ?(lev=get_current_level ()) ~env
                  ~expected_eff ~cont_ty expected_ty =
...
let type_pattern ?in_handler ~lev ~allow_exn env expected_eff spat scope
                 cont_ty expected_ty =
...
let rec type_approx env sexp =
  ...
  | Pexp_function ({pc_rhs=e}::_) ->
      ...
      let ty2 = newvar Seffect in
      let ty3, closable = type_approx env e in
      let ty = newty (Tarrow(Nolabel, ty1, ty2, ty3, Cok)) in
      ty, closable
...
let rec type_exp ?recarg env expected_eff sexp =
  (* We now delegate everything to type_expect *)
  type_expect ?recarg env expected_eff sexp (newvar Stype)
...
and type_expect ?in_function ?recarg env expected_eff sexp ty_expected =
  let previous_saved_types = Cmt_format.get_saved_types () in
  Typetexp.warning_enter_scope ();
  Typetexp.warning_attribute sexp.pexp_attributes;
  let exp =
    type_expect_ ?in_function ?recarg env expected_eff sexp ty_expected
  in
...
and type_expect_ ?in_function ?(recarg=Rejected) env expected_eff sexp
                 ty_expected =
    ...
    let is_format = match ty_exp.desc with
      | Tconstr(path, _, _, _) when Path.same path fmt6_path ->
        if !Clflags.principal && ty_exp.level <> generic_level then
          Location.prerr_warning loc
            (Warnings.Not_principal "this coercion to format6e");
        true
      | _ -> false
    in
    if is_format then
  ...
  | Pexp_record(lid_sexp_list, opt_sexp) ->
      ...
      let opt_exp =
        ...
        | Some sexp ->
            if !Clflags.principal then begin_def ();
            let exp = type_exp ~recarg env expected_eff sexp in
            if !Clflags.principal then begin
              end_def ();
...
and type_label_access env loc expected_eff srecord lid =
  if !Clflags.principal then begin_def ();
  let record = type_exp ~recarg:Allowed env expected_eff srecord in
  if !Clflags.principal then begin
    end_def ();
...
and type_format loc str env =
  ...
  try
    CamlinternalFormatBasics.(CamlinternalFormat.(
      ...
      and mk_padding :
        type x y (p : effect) . (x, y, p) padding -> Parsetree.expression =
...
and type_argument ?recarg env expected_eff sarg ty_expected' ty_expected =
  ...
  | _ ->
      let texp = type_expect ?recarg env expected_eff sarg ty_expected' in
      unify_exp env texp ty_expected;
      texp
...
and type_construct env loc lid sarg ty_expected expected_eff attrs =
  ...
  in
  let args =
    List.map2 (fun e (t,t0) -> type_argument ~recarg env expected_eff e t t0)
...
and type_perform env loc expected_eff lid sarg attrs ty_expected =
  let ecstr = Typetexp.find_effect_constructor env lid.loc lid.txt in
...
and type_cases ?in_function ?in_handler ~allow_exn
               env expected_eff ty_arg ty_res loc caselist =
  ...
  let exp_list =
    List.map2
      (fun {pvb_expr=sexp; pvb_attributes; _} (pat, slot) ->
        ...
        | Tpoly (ty, tl) ->
            ...
            let exp =
              Typetexp.with_warning_attribute pvb_attributes (fun () ->
                type_expect exp_env expected_eff sexp ty')
            in
            end_def ();
        ...
        | _ ->
            Typetexp.with_warning_attribute pvb_attributes (fun () ->
              type_expect exp_env expected_eff sexp pat.pat_type))
...
let type_default_handler env expected_eff path seh =
  Typetexp.reset_type_variables ();
  let cases, _, handled =
    type_cases env ~allow_exn:true ~in_handler:path expected_eff
      (newvar Stype) (newvar Stype) seh.peh_loc seh.peh_cases
  in
...
let check_expectation env eff_expected =
  ...
  | () -> ()
  | exception Ctype.No_default_handler(p, loc, str) ->
      raise (Error(loc, env, Toplevel_no_default_handler(p, str)))
...
let report_error env ppf = function
  ...
  | Default_handler_nonreturning lid ->
      fprintf ppf
        ...
        longident lid
  | Toplevel_no_default_handler(p, str) ->
      fprintf ppf
        "@[This %s performs effect %a, which has no default handler.@]"
        str Printtyp.path p
