...
let type_module =
  ref ((fun eff env md -> assert false) :
...
let unify_pat_inner_effects loc env eff inner_eff =
  try
    unify env eff inner_eff
  with
  | Unify trace ->
      raise(Error(loc, env, Pattern_inner_effect_clash(trace)))
  | Effect_tags(l1, l2) ->
      raise(Typetexp.Error(loc, env, Typetexp.Effect_tags (l1, l2)))

let unify_pat_outer_effects loc env eff outer_eff =
  try
    unify env eff outer_eff
  with
  | Unify trace ->
      raise(Error(loc, env, Pattern_outer_effect_clash(trace)))
  | Effect_tags(l1, l2) ->
      raise(Typetexp.Error(loc, env, Typetexp.Effect_tags (l1, l2)))
...
let unify_pat_inner_eff env pat inner_eff =
  unify_pat_inner_effects pat.pat_loc env pat.pat_inner_eff inner_eff

let unify_pat_outer_eff env pat outer_eff =
  unify_pat_outer_effects pat.pat_loc env pat.pat_outer_eff outer_eff
...
let rec build_as_type env p =
  ...
  | Tpat_record (lpl,_) ->
      ...
      let do_label lbl =
        let _, ty_arg, ty_res, ty_mut = instance_label false lbl in
        unify_pat env {p with pat_type = ty} ty_res;
        let refinable =
          ty_mut = None && List.mem_assoc lbl.lbl_pos ppl &&
          match (repr lbl.lbl_arg).desc with Tpoly _ -> false | _ -> true in
        if refinable then begin
          let arg = List.assoc lbl.lbl_pos ppl in
          unify_pat env {arg with pat_type = build_as_type env arg} ty_arg
        end else begin
          let _, ty_arg', ty_res', _ = instance_label false lbl in
...
let build_or_pat env loc inner_eff outer_eff lid =
      ...
      (fun (pats,fields) (l,f) ->
        ...
        | Rpresent (Some ty) ->
            (l, Some {pat_desc=Tpat_any; pat_loc=Location.none; pat_env=env;
                      pat_type=ty; pat_inner_eff=inner_eff;
                      pat_outer_eff=outer_eff; pat_extra=[];
                      pat_attributes=[]})
            :: pats,
    ...
    List.map
      (fun (l,p) ->
        {pat_desc=Tpat_variant(l,p,row'); pat_loc=gloc;
         pat_env=env; pat_type=ty; pat_inner_eff=inner_eff;
         pat_outer_eff=outer_eff; pat_extra=[]; pat_attributes=[]})
      pats
  in
  match pats with
        ...
        List.fold_left
          (fun pat pat0 ->
            {pat_desc=Tpat_or(pat0,pat,Some row0); pat_extra=[];
             ...
             pat_inner_eff=inner_eff; pat_outer_eff=outer_eff;
             pat_attributes=[]})
          pat pats in
...
let rec type_pat ~constrs ~labels ~no_existentials ~mode ~env
                 ~allow_exn ~outer_eff ~inner_eff ~cont_ty sp expected_ty =
  let type_pat ?(mode=mode) ?(env=env) =
    type_pat ~constrs ~labels ~no_existentials
      ~mode ~env ~allow_exn:false ~outer_eff ~inner_eff ~cont_ty
  in
  let loc = sp.ppat_loc in
  match sp.ppat_desc with
    Ppat_any ->
  ...
  | Ppat_alias(sq, name) ->
      ...
      rp {
        ...
        pat_loc = loc; pat_extra=[];
        pat_type = q.pat_type;
        pat_inner_eff = q.pat_inner_eff;
        pat_outer_eff = q.pat_outer_eff;
        pat_attributes = sp.ppat_attributes;
        pat_env = !env }
  | Ppat_constant cst ->
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      let type_label_pat (label_lid, label, sarg) =
        begin_def ();
        let (vars, ty_arg, ty_res, ty_mut) = instance_label false label in
        ...
        begin match ty_mut with
        | None -> ()
        | Some rg ->
            let eff = instance_def (effect_state rg (newvar Seffect)) in
            unify_pat_outer_effects loc !env eff outer_eff
        end;
  ...
  | Ppat_type lid ->
      let (path, p,ty) = build_or_pat !env loc inner_eff outer_eff lid.txt in
      unify_pat_types loc !env ty expected_ty;
      { p with pat_extra =
  ...
  | Ppat_effect(label, sargs, scont) ->
     ...
     in
     let args, res =
       instance_effect_constructor ~in_pattern:(env, get_newtype_level ()) ec
     in
...
let type_pat ?(allow_existentials=false) ?constrs ?labels
    ?(lev=get_current_level()) ~env ~allow_exn ~outer_eff
    ~inner_eff ~cont_ty sp expected_ty =
  newtype_level := Some lev;
  try
    let r =
      type_pat ~no_existentials:(not allow_existentials) ~constrs
        ~labels ~mode:Normal ~env ~allow_exn
        ~outer_eff ~inner_eff ~cont_ty sp expected_ty
    in
...
let partial_pred ~lev ~env ~outer_eff ~inner_eff ~cont_ty
      expected_ty constrs labels p =
  ...
  try
    ...
    let typed_p =
      type_pat ~allow_existentials:true ~lev
        ...
        ~outer_eff ~inner_eff ~cont_ty p expected_ty
    in
    backtrack snap;
...
let check_partial ?(lev=get_current_level ()) ~env
                  ~outer_eff ~inner_eff ~cont_ty expected_ty loc cases =
  let (partial, handled) =
    Parmatch.check_partial_gadt
      (partial_pred ~lev ~env ~outer_eff ~inner_eff ~cont_ty expected_ty)
      env loc cases
  in
  (partial, handled)
...
let type_pattern ~lev ~allow_exn env outer_eff inner_eff spat scope
                 cont_ty expected_ty =
  reset_pattern scope true;
  let new_env = ref env in
  let pat =
    type_pat ~allow_existentials:true ~lev ~env:new_env ~allow_exn
             ~outer_eff ~inner_eff ~cont_ty spat expected_ty
  in
  let new_env, unpacks =
...
let type_pattern_list ~allow_exn env outer_eff inner_eff spatl scope cont_ty
                      expected_tys allow =
  ...
  let patl =
    List.map2
      (fun spat expected_ty ->
        type_pat ~allow_exn ~env:new_env ~outer_eff ~inner_eff ~cont_ty
          spat expected_ty)
      spatl expected_tys
  in
...
let type_class_arg_pattern cl_num val_env met_env expected_eff l spat =
  reset_pattern None false;
  let nv = newvar Stype in
  let cont_ty = newvar Stype in
  let pat =
    type_pat ~allow_exn:false ~env:(ref val_env)
      ~outer_eff:expected_eff ~inner_eff:Ctype.none ~cont_ty spat nv
  in
...
let type_self_pattern cl_num privty val_env met_env par_env expected_eff spat =
...
let rec is_nonexpansive exp =
  ...
  | Texp_record(lbl_exp_list, opt_init_exp) ->
      List.for_all
        (fun (_, lbl, exp) ->
          lbl.lbl_mut = Lmut_immutable && is_nonexpansive exp)
        lbl_exp_list
...
let rec type_exp env expected_eff sexp =
  (* We now delegate everything to type_expect *)
  type_expect env expected_eff sexp (newvar Stype)
...
and type_expect ?in_function env expected_eff sexp ty_expected =
  let previous_saved_types = Cmt_format.get_saved_types () in
  Typetexp.warning_enter_scope ();
  Typetexp.warning_attribute sexp.pexp_attributes;
  let exp =
    type_expect_ ?in_function env expected_eff sexp ty_expected
  in
...
and type_expect_ ?in_function env expected_eff sexp ty_expected =
  ...
  let rufe exp =
    unify_exp env (re exp) (instance env ty_expected);
    unify_exp_effects exp.exp_loc env exp.exp_eff expected_eff;
    exp
  in
  match sexp.pexp_desc with
  | Pexp_ident lid ->
  ...
  | Pexp_constant(Const_string (str, _) as cst) -> (
    ...
    if is_format then
      ...
      type_expect ?in_function env expected_eff format_parsetree ty_expected
    else
      rue {
  ...
  | Pexp_let(Nonrecursive,
    ...
    (* TODO: allow non-empty attributes? *)
      type_expect ?in_function env expected_eff
        {sexp with
  ...
  | Pexp_let(rec_flag, spat_sexp_list, sbody) ->
      ...
      let (pat_exp_list, new_env, unpacks) =
        type_let env rec_flag spat_sexp_list scp true expected_eff in
      let body =
        type_expect new_env expected_eff
          (wrap_unpacks sbody unpacks) ty_expected
      in
      re {
  ...
  | Pexp_fun (l, Some default, spat, sexp) ->
      ...
      in
      type_expect ?in_function env expected_eff sfun ty_expected
  ...
  | Pexp_apply(sfunct, sargs) ->
      ...
      let funct = type_exp env expected_eff sfunct in
      ...
      begin_def ();
      let (args, ty_res) = type_application env loc expected_eff funct sargs in
      end_def ();
  ...
  | Pexp_match(sarg, caselist) ->
      ...
      unify_exp_effects arg.exp_loc env eff (newvar Seffect);
      if nonexpansive then begin
  ...
  | Pexp_tuple sexpl ->
      ...
      let expl =
        List.map2
          (fun body ty -> type_expect env expected_eff body ty)
          sexpl subtypes
      in
      re {
  ...
  | Pexp_construct(lid, sarg) ->
      type_construct env loc lid sarg ty_expected
        expected_eff sexp.pexp_attributes
  | Pexp_variant(l, sarg) ->
      (* Keep sharing *)
      ...
      | Some sarg, {desc = Tvariant row}, {desc = Tvariant row0} ->
          ...
          row_field_repr (List.assoc l row0.row_fields) with
            Rpresent (Some ty), Rpresent (Some ty0) ->
              let arg = type_argument env expected_eff sarg ty ty0 in
      ...
      with Not_found ->
        let arg =
          may_map
            (fun sexp -> type_exp env expected_eff sexp)
            sarg
        in
      ...
      let lbl_exp_list =
        wrap_disambiguate "This record expression is expected to have" ty_record
          (type_label_a_list loc closed env
             (type_label_exp true env loc expected_eff ty_record)
             opath)
          lid_sexp_list
      in
        ...
        | Some exp, (lid, lbl, lbl_exp) :: _ ->
            ...
            let unify_kept lbl =
              ...
              if List.for_all
                  ...
                  lbl_exp_list
              then begin
                let _, ty_arg1, ty_res1, ty_mut1 = instance_label false lbl
                and _, ty_arg2, ty_res2, ty_mut2 = instance_label false lbl in
                unify env ty_arg1 ty_arg2;
                unify env (instance env ty_expected) ty_res2;
                unify_exp_types exp.exp_loc env ty_exp ty_res1;
                match ty_mut1, ty_mut2 with
                | None, None -> ()
                | Some rg1, Some rg2 ->
                    let eff1 = instance_def (effect_state rg1 (newvar Seffect)) in
                    let eff2 = instance_def (effect_state rg2 (newvar Seffect)) in
                    unify_exp_effects loc env expected_eff eff2;
                    unify_exp_effects exp.exp_loc env eff_exp eff1;
                | _, _ -> assert false
              end in
  ...
  | Pexp_field(srecord, lid) ->
      ...
      let eff =
        match ty_mut with
        | Some rg ->
          instance_def (effect_state rg (newvar Seffect))
        | None ->
          newvar Seffect
      in
      rufe {
  ...
  | Pexp_setfield(srecord, lid, snewval) ->
      ...
      let (label_loc, label, newval) =
        type_label_exp false env loc expected_eff ty_record
          (lid, label, snewval)
      in
      unify_exp env record ty_record;
      if label.lbl_mut = Lmut_immutable then
  ...
  | Pexp_ifthenelse(scond, sifso, sifnot) ->
      let cond = type_expect env expected_eff scond Predef.type_bool in
      begin match sifnot with
        None ->
          let ifso = type_expect env expected_eff sifso Predef.type_unit in
          rue {
            ...
            exp_env = env }
      | Some sifnot ->
          let ifso = type_expect env expected_eff sifso ty_expected in
          let ifnot = type_expect env expected_eff sifnot ty_expected in
          (* Keep sharing *)
  ...
  | Pexp_sequence(sexp1, sexp2) ->
      let exp1 = type_statement env expected_eff sexp1 in
      let exp2 = type_expect env expected_eff sexp2 ty_expected in
      re {
  ...
  | Pexp_while(scond, sbody) ->
      let cond = type_expect env expected_eff scond Predef.type_bool in
      let body = type_statement env expected_eff sbody in
      rue {
  ...
  | Pexp_for(param, slow, shigh, dir, sbody) ->
      let low = type_expect env expected_eff slow Predef.type_int in
      let high = type_expect env expected_eff shigh Predef.type_int in
      let id, new_env =
      ...
      in
      let body = type_statement new_env expected_eff sbody in
      rue {
  ...
  | Pexp_constraint (sarg, sty) ->
      ...
      let (arg, ty') =
        if separate then begin
          ...
          (type_argument env expected_eff sarg ty ty, ty)
      in
      rue {
      ...
      let (arg, ty',cty,cty') =
        ...
        | Some sty ->
            ...
            if separate then begin
              ...
              (type_argument env expected_eff sarg ty ty,
               ty', Some cty, cty')
      in
      rue {
        exp_desc = arg.exp_desc;
  ...
  | Pexp_send (e, met) ->
      if !Clflags.principal then begin_def ();
      let obj = type_exp env expected_eff e in
      begin try
        let (meth, exp, typ) =
      ...
      begin try
        ...
        match desc.val_kind with
          Val_ivar (Mutable, cl_num) ->
            let newval =
              type_expect env expected_eff snewval (instance env desc.val_type)
            in
            let (path_self, _) =
              Env.lookup_value (Longident.Lident ("self-" ^ cl_num)) env
            in
        ...
        (path_self, _) ->
          let type_override (lab, snewval) =
            begin try
              ...
              let exp =
                type_expect env expected_eff snewval (instance env ty)
              in
  ...
  | Pexp_letmodule(name, smodl, sbody) ->
      ...
      let expectation = Ctype.effect_expectation expected_eff in
      let modl = !type_module env expectation smodl in
      ...
      re {
        ...
        exp_env = env }
  | Pexp_assert (e) ->
      let cond = type_expect env expected_eff e Predef.type_bool in
      let exp_type =
  ...
  | Pexp_newtype(name, ssort, sbody) ->
      ...
      let body = type_exp new_env expected_eff sbody in
  ...
  | Pexp_pack m ->
      ...
      in
      let expectation = Ctype.effect_expectation expected_eff in
      let (modl, tl') = !type_package env expectation m p nl tl in
      rue {
  ...
  | Pexp_open (ovf, lid, e) ->
      ...
      let exp = type_expect newenv expected_eff e ty_expected in
      { exp with
...
and type_function ?in_function loc attrs env ty_expected l caselist =
  ...
  let cases, partial, _ =
    type_cases ~allow_exn:false ~in_function:(loc_fun,ty_fun) env
      ty_eff Ctype.none ty_arg ty_res loc caselist
  in
  let not_function ty =
...
and type_label_access env loc expected_eff srecord lid =
  if !Clflags.principal then begin_def ();
  let record = type_exp env expected_eff srecord in
  if !Clflags.principal then begin
    end_def ();
...
and type_format loc str env =
  ...
  try
    CamlinternalFormatBasics.(CamlinternalFormat.(
      ...
      and mk_formatting_gen : type a b c d e f effect p.
      ...
      and mk_format : type a b c d e f effect p.
      ...
      and mk_fmtty : type a b c d e f g h i j k l effect p q.
      ...
      and mk_ignored : type a b c d e f effect p.
      ...
      and mk_padding : type x y effect p. (x, y, p) padding -> Parsetree.expression =
      ...
      and mk_precision : type x y effect p. (x, y, p) precision -> Parsetree.expression =
      ...
      and mk_fmt : type a b c d e f effect p.
...
and type_label_exp create env loc expected_eff ty_expected
          (lid, label, sarg) =
  (* Here also ty_expected may be at generic_level *)
  begin_def ();
  let separate = !Clflags.principal || Env.has_local_constraints env in
  if separate then (begin_def (); begin_def ());
  let (vars, ty_arg, ty_res, ty_mut) = instance_label true label in
  if separate then begin
    end_def ();
...
and type_argument env expected_eff sarg ty_expected' ty_expected =
  ...
  match expand_head env ty_expected' with
    {desc = Tarrow("",ty_arg,_,ty_res,_); level = lv} when is_inferred sarg ->
      ...
      if !Clflags.principal then begin_def ();
      let texp = type_exp env expected_eff sarg in
      if !Clflags.principal then begin
        end_def ();
      ...
      let var_pair name ty =
        ...
        {pat_desc = Tpat_var (id, mknoloc name); pat_type = ty;
         pat_inner_eff = Ctype.none; pat_outer_eff = expected_eff;
  ...
  | _ ->
      let texp = type_expect env expected_eff sarg ty_expected' in
      unify_exp env texp ty_expected;
      texp
...
and type_application env loc expected_eff funct sargs =
  ...
  let rec type_unknown_args
    ...
    | (l1, sarg1) :: sargl ->
        ...
        let arg1 () =
          let arg1 = type_expect env expected_eff sarg1 ty_arg in
        ...
        let sargs, more_sargs, arg =
          if ignore_labels && not (is_optional l) then begin
            ...
            | _, (l', sarg0) :: more_sargs ->
                if l <> l' && l' <> "" then
                  ...
                  ([], more_sargs,
                   Some (fun () ->
                           unify_effect sarg0;
                           type_argument env expected_eff sarg0 ty ty0))
            | _ ->
                assert false
          end else try
            ...
            if optional = Required || is_optional l' then
              ...
              Some (fun () ->
                      unify_effect sarg0;
                      option_some (type_argument env expected_eff sarg0
  ...
  match funct.exp_desc, sargs with
    ...
    ["", sarg] ->
      ...
      in
      let exp = type_expect env expected_eff sarg ty_arg in
...
and type_perform env loc expected_eff label sargs returns attrs ty_expected =
  ...
  let ec =
    ...
    | Effect_tags(l1, l2) ->
        raise(Typetexp.Error(loc, env, Typetexp.Effect_tags (l1, l2)))
  in
  let args, res = instance_effect_constructor ec in
  Misc.may
    (fun res -> unify_exp_types loc env res (instance env ty_expected))
    res;
  (* Type check arguments *)
  let targs =
    List.map2
...
and type_statement env expected_eff sexp =
      ...
      (fun {pc_lhs; pc_guard; pc_rhs} ->
        ...
        let (pat, ext_env, force, unpacks) =
          ...
          type_pattern ~lev ~allow_exn
            env outer_eff inner_eff pc_lhs scope ty_res ty_arg
        in
        pattern_force := force @ !pattern_force;
        let pat =
          if !Clflags.principal then begin
...
(*        Format.printf "@[%i %i, ty_res' =@ %a@]@." lev (get_current_level())
  ...
  let partial, handled =
    check_partial ~lev ~env ~outer_eff ~inner_eff
      ~cont_ty:ty_res ty_arg loc cases
  in
  add_delayed_check
    (fun () ->
...
and type_let ?(check = fun s -> Warnings.Unused_var s)
  ...
  let (pat_list, new_env, force, unpacks) =
    type_pattern_list ~allow_exn:false env expected_eff Ctype.none spatl scope
      cont_ty nvs allow
  in
  let closable =
    ...
    List.map2
      (fun {pvb_expr=sexp; pvb_attributes; _} (pat, slot, typ) ->
        ...
        | Tpoly (ty, tl) ->
            ...
            unify_exp_effects exp.exp_loc env eff (newvar Seffect);
            if not nonexpansive then begin
  ...
  List.iter2
    (fun pat (exp, _) ->
      ignore(check_partial ~env ~outer_eff:expected_eff ~inner_eff:Ctype.none
               ~cont_ty pat.pat_type pat.pat_loc [case pat exp]))
    pat_list exp_list;
  end_def();
  let nonexpansive =
    List.map
      (fun (exp, eff) -> pure_effect env eff || is_nonexpansive exp)
      exp_list
  in
  List.iter
    (fun (exp, eff) ->
      unify_exp_effects exp.exp_loc env eff expected_eff)
    exp_list;
  List.iter2
    (fun pat nonexpansive ->
      if not nonexpansive then
        iter_pattern (fun pat -> generalize_expansive env pat.pat_type) pat)
    pat_list nonexpansive;
  List.iter
...
let type_binding env loc expected_eff rec_flag spat_sexp_list scope =
  ...
  let expected_eff = expectation_effect "binding" env loc expected_eff in
  let (pat_exp_list, new_env, unpacks) =
    type_let
      ~check:(fun s -> Warnings.Unused_value_declaration s)
      ~check_strict:(fun s -> Warnings.Unused_value_declaration s)
      env rec_flag spat_sexp_list scope false expected_eff
  in
  (pat_exp_list, new_env)

let type_let env loc expected_eff rec_flag spat_sexp_list scope =
  let expected_eff = expectation_effect "let binding" env loc expected_eff in
  let (pat_exp_list, new_env, unpacks) =
...
let type_expression env expected_eff sexp =
  Typetexp.reset_type_variables();
  let expected_eff =
    expectation_effect "expression" env sexp.pexp_loc expected_eff
  in
  begin_def();
  let eff = newvar Seffect in
...
let check_expectation expected_eff =
  ...
  | exception Ctype.Unknown_region(env, ty, loc, str) ->
      raise (Error(loc, env, Toplevel_unknown_region(ty, str)))
...
let report_error env ppf = function
  ...
  | Pattern_inner_effect_clash trace ->
      report_unification_error ppf env trace
        (function ppf ->
           fprintf ppf "This pattern handles effect")
        (function ppf ->
           fprintf ppf "but a pattern was expected which handles")
  | Pattern_outer_effect_clash trace ->
      report_unification_error ppf env trace
        (function ppf ->
           fprintf ppf "This pattern performs effect")
        (function ppf ->
           fprintf ppf "but a pattern was expected that performed")
  ...
  | Toplevel_unknown_region(ty, str) ->
      fprintf ppf
        "@[This %s performs state effects in region %a, which may not be global.@]"
        str Printtyp.type_sch ty
