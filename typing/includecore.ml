 ...
 *       pr "The argument types for constructor %s are not equal" s
 *   | Effect_constructor_ret_type s ->
 *       pr "The return types for constructor %s are not equal" s
 *   | Effect_constructor_arity s ->
 *       pr "The arities for constructor %s differ" s
 ...
 *        s (if b then second else first) eff
 * 
 ...
 *   | [], []           -> []
 *   | [], ec::_ -> [Effect_constructor_missing (true, ec.Types.ec_name)]
 *   | ec::_, [] -> [Effect_constructor_missing (false, ec.Types.ec_name)]
 *   | {Types.ec_name=ec1; ec_args=arg1; ec_res=ret1}::rem1,
 *     {Types.ec_name=ec2; ec_args=arg2; ec_res=ret2}::rem2 ->
 *       if ec1 <> ec2 then
