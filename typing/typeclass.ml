...
let rec constructor_type constr cty =
  ...
  | Cty_signature sign ->
      constr
  | Cty_arrow (l, ty, cty) ->
      let eff = Ctype.instance_def (Ctype.Effrow.with_io (Ctype.newty Tenil)) in
...
let rec class_field self_loc cl_num self_type meths vars
  ...
  | Pcf_val (lab, mut, Cfk_concrete (ovf, sexp)) ->
      ...
      if !Clflags.principal then Ctype.begin_def ();
      let eff = Ctype.Effrow.with_io (Btype.newgenty Tenil) in
      let exp =
...
and class_structure cl_num final val_env met_env loc
  ...
  (* Self binder *)
  let self_eff = Ctype.instance_def (Ctype.Effrow.with_io (Ctype.newty Tenil)) in
...
and class_expr cl_num val_env met_env scl =
  ...
  | Pcl_fun (l, None, spat, scl') ->
      if !Clflags.principal then Ctype.begin_def ();
      let eff =
        Ctype.instance_def (Ctype.Effrow.with_io (Ctype.newty Tenil))
      in
